...
async function uploadChrome() {
  ...
  const zipFiles = await glob(`./web-ext-artifacts/chrome/**/*-${manifest.version}.zip`, {
    posix: true,
    dotRelative: true,
  });

  if (zipFiles.length === 0) {
    throw new Error(`Can't find a .zip file with version ${manifest.version}. Check that pnpm zip:chrome has been run.`);
  }

  if (zipFiles.length > 1) {
    throw new Error(`There are duplicates .zip files with version ${manifest.version}.`);
  }
